...
export class StorageController {
  ...
  async download( @Query('fileName') fileName: string,
    ...
    try {
      ...
      const { content, url } = await this.storageService.download(fileName, viewerId, publisher);
      ...
      if (error instanceof FileNotFoundException) {
        this.logger.error('File not found:', error);
          ...
          this.logger.error(`The user ${viewerId} is not authorized to access the file ${fileName}.`);
      }
      else {
